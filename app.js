...
function renderPack(id) { const p = TRAINING_PACKS.find(x=>x.id===id); if(!p)return; setView('pack'); $('#pack-container').innerHTML = `<div class="panel"><p class="section-label">${p.label}</p><h2>${p.title}</h2><p class="muted">${p.source}</p>${p.questions.map((q,i)=>`<article class="question-card"><p>第 ${i+1} 題</p><h3>${q.q}</h3>${q.c.map((c,n)=>`<button class="choice" data-answer="${n}" data-pack="${p.id}" data-question="${i}">${String.fromCharCode(65+n)}．${c}</button>`).join('')}<p class="feedback"></p></article>`).join('')}</div>`; }
function answerPack(button) { const p=TRAINING_PACKS.find(x=>x.id===button.dataset.pack), q=p.questions[+button.dataset.question], card=button.closest('.question-card'), selected=+button.dataset.answer; card.querySelectorAll('.choice').forEach(b=>{b.disabled=true;if(+b.dataset.answer===q.a)b.style.borderColor='#2c7c61'}); button.style.borderColor=selected===q.a?'#2c7c61':'#b84343'; card.querySelector('.feedback').textContent=selected===q.a?`正確。${q.e}`:`正解：${String.fromCharCode(65+q.a)}。${q.e}`; }
