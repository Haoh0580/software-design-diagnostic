...
const PACK_TOPICS = {
  'csharp-core':['C#','Array / 資料結構'], 'logic-algorithm':['Debug','Algorithm'],
  'pro2-mock':['數位邏輯','微處理機','程式設計實習'], 'pro1-rebuild':['基本電學','電子學']
};
const EXAM_TOPICS = ['基本電學','電子學','微處理機','數位邏輯','程式設計實習'];
...
function trainingMistakes() { const answers=dash.packAnswers||{}; return TRAINING_PACKS.flatMap(pack => pack.questions.map((q,index) => ({pack,q,index,answer:answers[pack.id]?.[index]})).filter(item => item.answer !== undefined && item.answer !== item.q.a)); }
function renderMistakes() { const mistakes=trainingMistakes(); $('#mistakes-view').innerHTML = `<div class="panel"><p class="section-label">Mistakes</p><h2>錯題整理</h2>${mistakes.length ? `<p class="muted">目前有 ${mistakes.length} 題訓練題需複習。</p>${mistakes.map((item,i)=>`<article class="question-card"><p class="section-label">${item.pack.title} · 錯題 ${i+1}</p><h3>${item.q.q}</h3><p>你的答案：${String.fromCharCode(65+item.answer)}．${item.q.c[item.answer]}</p><p><strong>正確答案：${String.fromCharCode(65+item.q.a)}．${item.q.c[item.q.a]}</strong></p><p class="muted">解析：${item.q.e}</p></article>`).join('')}` : '<p>目前沒有已記錄的錯題。完成題組後，答錯的選擇題會出現在這裡。</p>'}<button class="button full" data-nav="library">回到題庫練習</button></div>`; }
function renderProgress() {
  const logs=dash.trainingLog||[], now=new Date(), year=now.getFullYear(), month=now.getMonth(), days=new Date(year,month+1,0).getDate(), first=new Date(year,month,1).getDay();
  const byDay=logs.reduce((map,log)=>((map[log.date] ||= []).push(log),map),{}); const mistakes=trainingMistakes();
  const practiced=new Set(logs.flatMap(log=>PACK_TOPICS[log.packId]||[])); const topicRows=EXAM_TOPICS.map(topic=>({topic,done:practiced.has(topic)}));
  const weakTopics={}; mistakes.forEach(item=>(PACK_TOPICS[item.pack.id]||['其他']).forEach(topic=>weakTopics[topic]=(weakTopics[topic]||0)+1));
  const weakEntries=Object.entries(weakTopics).sort((a,b)=>b[1]-a[1]); const totalWeak=weakEntries.reduce((n,[,v])=>n+v,0);
  const colors=['#bf4b4b','#d58a24','#3f789b','#76549a','#4e896a']; const slices=weakEntries.map(([topic,count],i)=>`${colors[i%colors.length]} ${Math.round(weakEntries.slice(0,i).reduce((n,[,v])=>n+v,0)/totalWeak*360)}deg ${Math.round(weakEntries.slice(0,i+1).reduce((n,[,v])=>n+v,0)/totalWeak*360)}deg`).join(',');
  const blanks=Array(first).fill('<span class="calendar-blank"></span>').join(''); const calendar=Array.from({length:days},(_,i)=>{const date=`${year}-${String(month+1).padStart(2,'0')}-${String(i+1).padStart(2,'0')}`, entries=byDay[date]||[], correct=entries.reduce((n,x)=>n+x.correct,0), total=entries.reduce((n,x)=>n+x.total,0); return `<article class="calendar-day ${entries.length?'has-study':''}"><b>${i+1}</b>${entries.length?`<small>${total} 題</small><em>${Math.round(correct/total*100)}%</em>`:'<small>—</small>'}</article>`}).join('');
  $('#progress-view').innerHTML=`<div class="panel"><p class="section-label">Progress</p><h2>${year} 年 ${month+1} 月學習月曆</h2><p class="muted">有色日期代表完成過題組；顯示該日完成題數與正確率。</p><div class="calendar-week"><span>日</span><span>一</span><span>二</span><span>三</span><span>四</span><span>五</span><span>六</span></div><div class="calendar-grid">${blanks}${calendar}</div></div><div class="panel"><p class="section-label">Mock Exam Readiness</p><h2>一模／二模重點</h2><p class="muted">已練習與尚未練習分開顯示，避免把「沒有資料」誤判成熟練。</p><div class="readiness-list">${topicRows.map(x=>`<div><span>${x.topic}</span><b class="${x.done?'ready':'not-started'}">${x.done?'已有練習紀錄':'尚未練習'}</b></div>`).join('')}</div></div><div class="panel"><p class="section-label">Weakness Map</p><h2>錯題弱項分析</h2>${weakEntries.length?`<div class="chart-row"><div class="donut" style="background:conic-gradient(${slices})"><span>${totalWeak}<small>錯題</small></span></div><div class="legend">${weakEntries.map(([topic,count],i)=>`<p><i style="background:${colors[i%colors.length]}"></i>${topic}<b>${count} 題</b></p>`).join('')}</div><p class="muted">優先加強：${weakEntries[0][0]}。另有 ${topicRows.filter(x=>!x.done).length} 個模考重點尚未練習。</p>`:'<p>完成並答錯訓練題後，這裡會用圖表顯示需要加強的觀念。</p>'}</div><div class="panel"><p class="section-label">Skill Check</p><h2>Diagnostic Test v1</h2><p>保留原本 Part A～D 與完整診斷報告。</p><button id="open-diagnostic" class="button full">開始／查看 Diagnostic Test</button></div>`;
}
...
function finishPack(id) { const p=TRAINING_PACKS.find(x=>x.id===id), answers=(dash.packAnswers||{})[id]||{}, answered=Object.keys(answers).length; if(answered<p.questions.length){$('#pack-status').textContent=`尚有 ${p.questions.length-answered} 題未作答，完成後才能查看摘要。`;return;} const correct=p.questions.filter((q,i)=>answers[i]===q.a).length; dash.trainingLog ||= []; const date=todayKey(), entry={date,packId:id,total:p.questions.length,correct}; const existing=dash.trainingLog.findIndex(log=>log.date===date&&log.packId===id); if(existing>=0)dash.trainingLog[existing]=entry;else dash.trainingLog.push(entry); saveDash(); $('#pack-status').innerHTML=`<strong>本組完成：${correct}/${p.questions.length} 題正確（${Math.round(correct/p.questions.length*100)}%）。</strong> 已寫入 ${date} 的月曆與錯題弱項分析。`; }
