const TEST_PARTS = [
  ...
  },
  {
    ...
    questions: [
      ...
      { id: 'b3', type: 'short', category: '邊界條件', prompt: '若二分搜尋的 right 初始值設為 arr.Length（而不是 arr.Length - 1），可能造成什麼問題？', keywords: ['index', '範圍', 'out', '越界'] },
      ...
      { id: 'b5', type: 'short', category: '除錯策略', prompt: '程式偶爾輸出錯誤答案。請寫出你會先檢查的一個具體位置或方法。', keywords: ['測資', '輸入', '邊界', 'print', 'console', '中間'] }
    ]
  },
  {
    ...
    questions: [
      ...
      { id: 'c4', type: 'short', category: '解題拆解', prompt: '給一串整數，找出是否有重複值。請用一句話描述一個 O(n) 左右的做法。', keywords: ['hashset', 'set', '集合', '加入', '出現'] },
      { id: 'c5', type: 'short', category: '邊界條件', prompt: '讀入 N 筆資料後要找最大值。請列出至少一個你會特別測試的邊界情況。', keywords: ['1', '空', '負', '相同', '最大', '最小'] }
    ]
  },
  {
    ...
    questions: [
      { id: 'd1', type: 'long', category: '實作規劃', prompt: '題目：讀入 N 與 N 個整數，輸出不重複數字的個數。\n\n請寫出你的 C# 解題步驟或程式骨架。', keywords: ['hashset', 'set', 'readline', 'split', 'add', 'count'] },
      { id: 'd2', type: 'long', category: '實作規劃', prompt: '題目：給一個字串，判斷它是否為迴文（正讀、反讀相同）。\n\n請寫出你的實作策略，或 C# 程式骨架。', keywords: ['left', 'right', 'while', 'char', 'length', 'i', 'j'] },
      { id: 'd3', type: 'long', category: '測試與除錯', prompt: '上面任選一題：請列出至少兩個你會自己測試的測資或邊界情況。', keywords: ['空', '1', '單', '重複', '相同', '大小寫', '偶數', '奇數'] }
    ]
  }
];
